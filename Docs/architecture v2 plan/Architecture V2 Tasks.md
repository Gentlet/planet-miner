...
- [ ] **Task 6.5: 연결 변경 및 잔류 아이템 처리**
  - 선행 조건: Task 6.3~6.4.
  - 구현 범위·상태 소유자: 선택된 입출구 제거·재연결과 교차점 안의 잔류 아이템을 처리한다. 라우팅 참조와 실제 아이템 소유권을 구분한다.
  - 후속 연결: 실제 철거 요청과의 연결은 Phase 7에서 완료한다.
  - 검증: 이동 전후 연결 제거, 대체 포트 생성, 잔류 아이템 배출과 공간 인덱스 동기화를 검사한다.
  - 완료 기준: 연결 변경으로 아이템 유실·중복·유효하지 않은 포트 참조가 남지 않는다.

- [ ] **Task 6.6: 전체 물류 파이프라인 통합 검증**
  - 선행 조건: Task 6.2~6.5 및 생산물 출력에 사용하는 기존 구현의 보완 검증.
  - 구현 범위·상태 소유자: 채굴·제작·창고·벨트·분배·합류를 실제 실행 Group으로 연결해 검증한다. 검증용 새 상태 소유자를 만들지 않는다.
  - 후속 연결: 설치·철거 중 물류 변화는 Phase 7의 수명주기 검증에 연결한다.
  - 검증: 연속 흐름, 역류 정체, 목적지 경쟁, 연결 변경에서 아이템 보존과 공간·소유권 불변식을 검사한다.
